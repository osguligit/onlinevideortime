...
int setupStreamSocket(UsageEnvironment& env, Port port, ipv4AddressBits ip, int domain,
  ...
  if (domain == AF_INET) {
    ...
    if (port.num() != 0 || ReceivingInterfaceAddr != INADDR_ANY) {
      ...
      //MAKE_SOCKADDR_IN(name, ReceivingInterfaceAddr, port.num());
		MAKE_SOCKADDR_IN(name, ip, port.num());//skkang
      if (bind(newSocket, (struct sockaddr*)&name, sizeof name) != 0) {
	char tmpBuffer[100];
	sprintf(tmpBuffer, "IPv4 bind() error (port number: %d): ", ntohs(port.num()));
	socketErr(env, tmpBuffer);
...
Boolean writeSocket(UsageEnvironment& env,
  ...
  do {
    ...
    if (bytesSent != (int)bufferSize) {
	  ...
	  OutputDebugString(tmpBuf);//skkang
	  OutputDebugString("\n");//skkang
      break;
    }
