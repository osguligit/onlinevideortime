...
RTPSink::RTPSink(UsageEnvironment& env,
  ...
  fSSRC = our_random32();
#ifdef TIMEBASE_MODIFY
  fTimestampBase = 0; //skkang
#else
...
#ifdef TIMEBASE_MODIFY
  u_int32_t const rtpTimestamp = timestampIncrement; //skkang
#else
...
#ifdef TIMEBASE_MODIFY //skkang
  unsigned long	i_pts = (unsigned long)(getSendingStartPts() % 0x100000000);
  timeNow.tv_sec = (long)(i_pts / 1000);
  timeNow.tv_usec = (long)((i_pts % 1000) * 1000);
#else
