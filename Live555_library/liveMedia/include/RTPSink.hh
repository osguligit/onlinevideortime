...
private:
  ...
  //skkang
  int64_t*	    mPTS;
  u_int32_t     mStartRtpTimestamp;
  int		    mSending;
  int64_t*	    mSendingStartPTS;
  RTCPInstance*	mRTCPInstance;
  int		    mRtcpSent;
public:
	inline u_int32_t getTimestampBase() { return fTimestampBase; }
	inline u_int32_t getCurrentRTPTimestamp() { return fCurrentTimestamp; }
	inline int64_t*  getPtsPtr() { return mPTS; }
	inline void      setPtsPtr(int64_t* pPtsPtr) { mPTS = pPtsPtr; }
	inline void      setPts(int64_t pts) { *mPTS = pts; }
	inline void      setStartRtpTimestamp(u_int32_t nRtpTimestamp) { mStartRtpTimestamp = nRtpTimestamp; }
	inline u_int32_t getStartRtpTimestamp(u_int32_t nRtpTimestamp) {
		if (mSending == 0)
		{
			setStartRtpTimestamp(nRtpTimestamp);
			mSending = 1;
			return nRtpTimestamp;
		}
		return mStartRtpTimestamp;
	}
	inline void    setSendingStartPtsPtr(int64_t*	ptr) { mSendingStartPTS = ptr; }
	inline int64_t getSendingStartPts() { return *mSendingStartPTS; }
	inline RTCPInstance*	getRTCPInstance() { return mRTCPInstance; }
	inline void           setRTCPInstance(RTCPInstance* pRTCPInstance) { mRTCPInstance = pRTCPInstance; }
	inline int  getRtcpSent() { return mRtcpSent; }
	inline void setRtcpSent(int bSent = 1) { mRtcpSent = bSent; }
	inline void resetRtcpSent() { mRtcpSent = 0; }
};
