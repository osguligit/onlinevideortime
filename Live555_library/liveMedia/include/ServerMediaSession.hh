...
public:
	...
	int	fMulticastPort;
	int	fMulticastTTL;
	void setMulticastEnable(Boolean bEnable) { fMulticastEnable = bEnable; }
	inline Boolean getMulticastEnable() { return fMulticastEnable; }
	void setMulticastIP(char* strIP) {
		if (fMulticastIP == NULL)
		{
			fMulticastIP = new char[128];
			strcpy(fMulticastIP, strIP);
		}
	}
	inline char* getMulticastIP() { return fMulticastIP; }
	void setMulticastPort(int nPort) { fMulticastPort = nPort; }
	inline int getMulticastPort() { return fMulticastPort; }
	void setMulticastTTL(int nTTL) { fMulticastTTL = nTTL; }
	inline int getMulticastTTL() { return fMulticastTTL; }
	
	char* getStreamName() { return fStreamName; }
	unsigned getReferenceCount() { return fReferenceCount; }
		
	int* mTargetMessageThreadId;
	int mPauseMessage;
	int mResumeMessage;
	int mRTSPConnMessage;
	int mRTSPDissConnMessage;
	inline void setMessage(int* targetThreadId, int pauseMessage, int resumeMessage, int RtspConnMessage, int RtspDissConnMessage)
	{
		mPauseMessage = pauseMessage;
		mResumeMessage = resumeMessage;
		mRTSPConnMessage = RtspConnMessage;
		mRTSPDissConnMessage = RtspDissConnMessage;
		mTargetMessageThreadId = targetThreadId;
	}

	int m_nClientMAX;
	inline void setClientMAX(const int nClientMAX)
	{
		m_nClientMAX = nClientMAX;
	}
};
