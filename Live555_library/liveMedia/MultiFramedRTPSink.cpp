...
#if 1
//skkang
int RTP_PAYLOAD_MAX_SIZE = 1452;
int RTP_PAYLOAD_PREFERRED_SIZE = ((RTP_PAYLOAD_MAX_SIZE) < 1000 ? (RTP_PAYLOAD_MAX_SIZE) : 1000);
#else //original
...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  setPacketSizes((RTP_PAYLOAD_PREFERRED_SIZE), (RTP_PAYLOAD_MAX_SIZE));//skkang MTU
}
...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  if (durationInMicroseconds != 0)//skkang 20210325
  {
    this->mPacketDurationInMicro = durationInMicroseconds;
  }
  ...
  if (numFrameBytesToUse == 0 && frameSize > 0) {
    ...
    if (fOutBuf->isPreferredSize()
      ...
      // The packet is ready to be sent now
		if (fOutBuf->curPacketSize() == (12 + specialHeaderSize()))	// or if(frameSize==0),  12 : rtp header size
		{	
			//	frameSize == 0 means no payload and fOutBuf is actually empty even though fOutBuf->fCurOffset==16
			//  fOutBuf->curPacketSize() == (12 + specialHeaderSize()) means no payload
			//	AAC : specialHeaderSize()==4 ( because MPEG4GenericRTPSink::specialHeaderSize()==4)
			//  H.264 : specialHeaderSize()==0
			// 
			fOutBuf->resetOffset();
		}
      sendPacketIfNecessary();
