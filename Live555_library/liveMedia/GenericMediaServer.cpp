...
GenericMediaServer
::GenericMediaServer(UsageEnvironment& env, int ourSocketIPv4, int ourSocketIPv6, Port ourPort, /*ipv4AddressBits ourIP,*/
		     unsigned reclamationSeconds)
  : Medium(env),
    fServerSocketIPv4(ourSocketIPv4), fServerSocketIPv6(ourSocketIPv6),
    fServerPort(ourPort), /*fServerIP(ourIP),*/ fReclamationSeconds(reclamationSeconds),
...
int GenericMediaServer::setUpOurSocket(UsageEnvironment& env, Port& ourPort, ipv4AddressBits ourIP, int domain) {
  int ourSocket = -1;
  ...
  do {
    ...
    ourSocket = setupStreamSocket(env, ourPort, ourIP, domain, True, True);
...
GenericMediaServer::ClientSession::~ClientSession() {
  ...
  if (fOurServerMediaSession != NULL) {
	...
	//skkang
	char dbgmsg[100];
	sprintf(dbgmsg, "[%s] Disconnected - current clientCnt(%d)\r\n", fOurServerMediaSession->getStreamName(), fOurServerMediaSession->getReferenceCount());
	OutputDebugString(dbgmsg);
	PostThreadMessage(*fOurServerMediaSession->mTargetMessageThreadId, fOurServerMediaSession->mRTSPDissConnMessage, fOurServerMediaSession->getReferenceCount(), NULL);
    if (fOurServerMediaSession->referenceCount() == 0
	&& fOurServerMediaSession->deleteWhenUnreferenced()) {
      //fOurServer.removeServerMediaSession(fOurServerMediaSession);//skkang
