...
RTSPServer*
RTSPServer::createNew(UsageEnvironment& env, Port ourPort, ipv4AddressBits ourIP,
		      UserAuthenticationDatabase* authDatabase,
		      unsigned reclamationSeconds) {
  int ourSocketIPv4 = setUpOurSocket(env, ourPort, ourIP, AF_INET);
  int ourSocketIPv6 = setUpOurSocket(env, ourPort, ourIP, AF_INET6);
  if (ourSocketIPv4 < 0 && ourSocketIPv6 < 0) return NULL;
  
  //return new RTSPServer(env, ourSocketIPv4, ourSocketIPv6, ourPort, ourIP, authDatabase, reclamationSeconds);
...
Boolean RTSPServer::setUpTunnelingOverHTTP(Port httpPort) {
  fHTTPServerSocketIPv4 = setUpOurSocket(envir(), httpPort, INADDR_ANY, AF_INET);
  fHTTPServerSocketIPv6 = setUpOurSocket(envir(), httpPort, INADDR_ANY, AF_INET6);
...
RTSPServer::RTSPServer(UsageEnvironment& env,
		       int ourSocketIPv4, int ourSocketIPv6, Port ourPort, /*ipv4AddressBits ourIP,*/
		       UserAuthenticationDatabase* authDatabase,
		       unsigned reclamationSeconds)
  : GenericMediaServer(env, ourSocketIPv4, ourSocketIPv6, ourPort, /*ourIP,*/ reclamationSeconds),
...
void RTSPServer::RTSPClientConnection::handleCmd_MaxClient()
{
	// Don't do anything with "fCurrentCSeq", because it might be nonsense
	snprintf((char*)fResponseBuffer, sizeof fResponseBuffer,
		"RTSP/1.0 499 This connection is over than max connections\r\n\r\n");
}
...
::handleCmd_SETUP_afterLookup2(ServerMediaSession* sms) {
  do {
	...
	if (fOurServerMediaSession->referenceCount() >= fOurServerMediaSession->m_nClientMAX)//skkang, limit client increase
	{
		fOurClientConnection->handleCmd_MaxClient();
		break;
	}
	...
	//skkang
	char dbgmsg[100];
	sprintf(dbgmsg, "[%s] Connected - current clientCnt(%d)\r\n", fOurServerMediaSession->getStreamName(), fOurServerMediaSession->getReferenceCount());
	OutputDebugString(dbgmsg);
	PostThreadMessage(*fOurServerMediaSession->mTargetMessageThreadId, fOurServerMediaSession->mRTSPConnMessage, fOurServerMediaSession->getReferenceCount(), NULL);
    ...
    StreamingMode streamingMode = (StreamingMode)fOurRTSPServer.mDefaultStreamingMode;; //skkang
