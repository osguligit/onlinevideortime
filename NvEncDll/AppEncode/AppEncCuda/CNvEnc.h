...
public:
	...
	int m_nHeight;
	int m_nFps;
	char m_InputFormat[16];//NV_ENC_BUFFER_FORMAT -> BGRA, IYUV,...
	sEncConfig m_encConig;

	NvEncoderInitParam encodeCLIOptions;
	//NvEncoderOutputInVidMemCuda* pEnc;
	NvEncoderCuda* pEnc;
	CUdevice cuDevice = 0;
	CUcontext cuContextEnc = NULL;
	NV_ENC_BUFFER_FORMAT eFormat = NV_ENC_BUFFER_FORMAT_ARGB;

	CNvEnc(void* pParam = nullptr, CallBackEncoded* func = nullptr);
	~CNvEnc();

	template<class EncoderClass>
	void F_InitializeEncoder(EncoderClass &pEnc, NvEncoderInitParam encodeCLIOptions, NV_ENC_BUFFER_FORMAT eFormat, sEncConfig* _encConfig);

	int createEncoder(int iGpu, int nWidth, int nHeight, int nFps, char * inputFormat, sEncConfig* _encConfig);
	int encodeFrame(unsigned char * pInput, int nInputSize, unsigned char * pStream, int* nStreamSize, __int64 pts);
	int deleteEncoder();
	...
	HANDLE m_eGotData;
	HANDLE m_hThread;
	static unsigned __stdcall Thread_Encoding(void * pParam);
	void Thread_Encoding_Main();
	void* m_pParam;
	CallBackEncoded* m_func;
...
inline void ShowHelpAndExit(const char *szBadOption = NULL)
{
...
inline void ParseCommandLine(int argc, char *argv[], char *szInputFileName, int &nWidth, int &nHeight,
...
template<class EncoderClass>
inline void InitializeEncoder(EncoderClass &pEnc, NvEncoderInitParam encodeCLIOptions, NV_ENC_BUFFER_FORMAT eFormat)
{
...
inline void EncodeCuda(int nWidth, int nHeight, NV_ENC_BUFFER_FORMAT eFormat, NvEncoderInitParam encodeCLIOptions, CUcontext cuContext, std::ifstream &fpIn, std::ofstream &fpOut)
{
...
inline void EncodeCudaOpInVidMem(int nWidth, int nHeight, NV_ENC_BUFFER_FORMAT eFormat, NvEncoderInitParam encodeCLIOptions, CUcontext cuContext, std::ifstream &fpIn, std::ofstream &fpOut, char *outFilePath, int32_t cuStreamType)
{
...
inline int AppEncCuda_main(int argc, char **argv)
{
